...
-- Create the default admin user with fixed credentials
-- This will create the admin user if it doesn't exist in auth.users
-- Note: This requires manual setup in Supabase Auth since we can't create auth users via SQL

-- The admin email is: admin@movimientoconsolacion.com
-- The admin password is: Votaciones2025

-- Sample insert for when the admin user is created in auth:
